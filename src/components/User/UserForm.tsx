import { useAssignUserToRoleMutation, useCreateNewRoleMutation, useGetAllRolesQuery, useUnassignUserToRoleMutation, useUpdateUserMutation } from "@/pages/api/apiSlice"
import { Box, Button, Group, PasswordInput, Select, SimpleGrid, TextInput } from "@mantine/core"
...
export function CreateRoleForm({data}: any) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        createReturn?.succeeded ? notifications.show({
            title: 'Success',
            message:  createReturn?.message || 'Role Created  Successfully 👍',
            color: 'green',
        }) : notifications.show({
            title: 'Failure',
            message:  createReturn?.errors[0]?.description || createReturn?.data?.message || error?.data?.title || 'Error occurs when Role creating',
            color: 'red',
        })
    }
