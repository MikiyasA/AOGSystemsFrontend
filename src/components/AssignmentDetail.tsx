        ...
        >
            {detailData?.map((d: any) => {
                ...
                <Group display={'grid'} w={'max-content'} gap={2} style={{ alignContent:'baseline'}}>
                    ...
                    {d.key === "startDate" || d.key === "dueDate" || d.key === "expectedFinishedDate" 
                        || d.key === "finishedDate" ? (
                            (() => {
        ...
        <Box mt={10}>
            {detailData?.map((d: any) => (
                d.key === "description" && <>
                    ...
                    <div style={{ borderRadius: '10px', padding: '20px', width: '100%'}}  dangerouslySetInnerHTML={{ __html: data[d.key] }}></div>
                </>
            ))}
        </Box>
        </>
