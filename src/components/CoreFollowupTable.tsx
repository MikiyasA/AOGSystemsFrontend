...
import { IconSelector, IconChevronDown, IconChevronUp, IconSearch, IconEditCircle, IconEye, IconBolt } from '@tabler/icons-react';
import classes from '../styles/Followup.module.css';
import { useAddFollowUpMutation, useAddRemarkMutation, useGetAllActiveFollowUpsQuery, useGetAllCoreFollowupQuery, useGetPartByIdQuery } from '@/pages/api/apiSlice';
...
export function CoreFollowupTable({data, table, tableTitle, isActive}: any) {
  ...
  return(
    <Table.Tr key={index} style={{borderRadius: 10}}
      ...
      {table?.map((col: any, index: any) => {
        if (col.key === "airCraft") {
          return <Table.Td key={`col-${index}`} p={5} m={0}>{row[col.key]} - {row.tailNo}</Table.Td>
        } else if (col.key === "poCreatedDate" || col.key === "partReceiveDate" || col.key === "returnDueDate" 
        || col.key === "returnProcessedDate" || col.key === "podDate") {
          console.log('typeof', typeof col.key, col.key)
      ...
      <Table.Td display={'flex'}>
        ...
        <IconEditCircle cursor={'pointer'} color='green' onClick={() => 
           modals.open({
            ...
            children: (
              <CoreForm data={row} action='update'/>
            ),
           })
        } />
        <ActionMenu row={row}/>
  ...
  const filterForm = useForm({
    ...
    validate: {
      pn: (v: string) => (v?.length < 4 ? 'PN must be provided and should be 4 character minimum' : null),
    }
  })
  ...
  table = tableData
  const {data: filterQueryData, isLoading: filterIsLoading, refetch} = useGetAllCoreFollowupQuery(filterForm.values)
  ...
  return (
    ...
    <ScrollArea>
      ...
      {!isActive &&
        ...
        <form onSubmit={handleFilterSubmit}>
          ...
          >
            ...
            <Group style={{display: 'grid', gridTemplateColumns: '50% 50%', gap: '3px'}}>
              <DateInput
                label='From'
                valueFormat='DD-MMM-YYYY'
                placeholder='Pick Start Date to filtered by Created Date'
                onChange={(e: any) => filterForm.setValues({createdStartDate: new Date(e?.toString())})}
              ...
              <DateInput
                ...
                onChange={(e: any) => filterForm.setValues({createdEndDate: new Date(e.toString())})}
            ...
            <Group style={{display: 'grid', gridTemplateColumns: '50% 50%', gap: '3px'}}>
              <DateInput
                ...
                onChange={(e: any) => filterForm.setValues({returnStartDate: new Date(e.toString())})}
              ...
              <DateInput
                ...
                onChange={(e: any) => filterForm.setValues({returnEndDate: new Date(e.toString())})}
            ...
            <Group style={{display: 'grid', gridTemplateColumns: '50% 50%', gap: '3px'}}>
              <DateInput
                ...
                onChange={(e: any) => filterForm.setValues({podStartDate: new Date(e.toString())})}
              ...
              <DateInput
                ...
                onChange={(e: any) => filterForm.setValues({podEndDate: new Date(e.toString())})}
            ...
            />
            <Button type="submit" mt="sm" loading={false}> Submit</Button>
      ...
      <Table.ScrollContainer minWidth={200}>
        <Table horizontalSpacing="lg" verticalSpacing="xs"  layout="fixed" highlightOnHover striped>
          <Table.Tbody>
            <Table.Tr>
              <Th>Item#</Th>
              {table?.map((col: any, index: any) => {
