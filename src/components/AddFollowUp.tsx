import { useAddFollowUpMutation, useAddRemarkMutation, useGetAllActiveFollowUpsTabsQuery } from "@/pages/api/apiSlice";
...
const FollowUpForm = ({tab}: any) => {
  ...
  const form = useForm({
    ...
    validate: {
      rid: (v) => (v?.length < 3 ? 'RID must be provided and should be 3 character minimum' : null),
        }
    })
    
    const [addFollowup, {isSuccess: isFpSuccess, isError, error: upFperror,isLoading:isFpLoading}] = useAddFollowUpMutation()
    ...
    return (
        ...
        <SimpleGrid
            ...
            <Select
                ...
                placeholder="Status"
                data={['Request Received', 'On Quote', 'Order Placed', 'On Hold', 'Under Shipping', 'Under Recieving', 'Received', 'Received & Advised', 'Closed']}
                searchable
                nothingFoundMessage="Nothing found..."
