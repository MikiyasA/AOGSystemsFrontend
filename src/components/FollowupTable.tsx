...
import { useAddFollowUpMutation, useAddRemarkMutation, useGetAllActiveFollowUpsQuery, useGetPartByIdQuery } from '@/pages/api/apiSlice';
    ...
    <Table.Tr key={index} style={{ backgroundColor: getColor(row.id) }}>
      ...
      {table?.map((col: any, index: any) => {
        if (col.key === "remarks" && row[col.key][0]) {
          return <Table.Td key={`col-${index}`} p={5} m={0}>
          ...
          return <Table.Td key={`col-${index}`} p={5} m={0}>
            <Link href={`/part/detail/${row.partNumber}`} style={{textDecoration: 'none', color: 'inherit'}} >{row.partNumber}</Link>
            </Table.Td>
        } else if (col.key === "quantity") {
          return <Table.Td key={`col-${index}`} p={5} m={0}>{row[col.key]} {row.uom}</Table.Td>
