...
import CostSavingDetail from "./CostSavingDetail";
import CostSavingForm from "./CostSavingForm";
...
export function CostSavingTable({
  data,
  table,
  tableTitle,
  isActive,
    ...
    return (
      <Table.Tr key={index} style={{ borderRadius: 10 }}>
        ...
        {table?.map((col: any, index: any) => {
          ...
          } else if (col.key === "isRepairOrder") {
            return (
              <Table.Td key={`col-${index}`} p={5} m={0}>
                {row.isRepairOrder && <IconCircleCheck color="green" />}
              </Table.Td>
            );
          } else if (col.key === "isPurchaseOrder") {
            return (
              <Table.Td key={`col-${index}`} p={5} m={0}>
                {row.isPurchaseOrder && <IconCircleCheck color="green" />}
              </Table.Td>
            );
          } else {
            return (
          ...
          <Table.Td display={"flex"}>
            <IconEye
              ...
              onClick={() => {
                modals.open({
                  size: "100%",
                  title: "Cost Saving Detail",
                  children: (
                    <CostSavingDetail data={row} detailData={detailData} />
                  ),
                });
              }}
            />
            <WithComponentAuth allowedRoles={["Coordinator", "TL"]}>
              <IconEditCircle
                ...
                onClick={() =>
                  modals.open({
                    size: "100%",
                    title: "Update Cost Saving",
                    children: <CostSavingForm data={row} action="update" />,
                  })
                }
              />
            </WithComponentAuth>
          ...
          <WithComponentAuth allowedRoles={["Coordinator", "TL"]}>
            <Button
              onClick={() =>
                modals.open({
                  size: "100%",
                  title: "Add Cost Saving",
                  children: <CostSavingForm data={data} action="add" />,
                })
              }
            >
              Add Cost Saving
